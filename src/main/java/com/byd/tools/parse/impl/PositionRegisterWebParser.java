...
/**
 * ClassName: PositionRegisterWebParser
 ...
 * Version 1.0
 */
public class PositionRegisterWebParser extends BaseWebParser {
    @Override
    public List<Comment> parseDataFromHtml(InputStream sourceInputStream, String charset, String sourceUrl) throws IOException {
        ...
        try {
            ...
            trs.forEach(
                    tr -> {
                        String positionRegisterName = tr.select("td").get(1).select("input").first().attr("name");
                        String positionRegisterValue = tr.select("td").get(1).select("input").first().attr("value");
                        Comment positionRegisterComment = new Comment(Integer.parseInt(positionRegisterName.substring(10)), positionRegisterValue); //通过name与value信息构造长文本对象。
                        positionRegisterComment.setType(CommentType.POSITION_REGISTER); //设置长文本类型为， 位置寄存器类型
                        commentList.add(positionRegisterComment);//将该行所提取出来的输入长文本加入到长文本列表:commentList当中
                    }
            );
