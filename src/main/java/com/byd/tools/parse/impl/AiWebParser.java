 ...
 * Version 1.0
 */
public class AiWebParser extends BaseWebParser  {
    @Override
    public List<Comment> parseDataFromHtml(InputStream sourceInputStream, String charset, String sourceUrl) throws IOException {
        ...
        try {
            ...
            trs.forEach(
                    tr -> {
                        String aiCommentName = tr.select("td").get(1).select("input").first().attr("name");
                        String aiCommentValue = tr.select("td").get(1).select("input").first().attr("value");
                        Comment aiComment = new Comment(Integer.parseInt(aiCommentName.substring(10)), aiCommentValue); //通过name与value信息构造长文本对象。
                        aiComment.setType(CommentType.AI); //设置长文本类型为， AI类型
                        commentList.add(aiComment);//将该行所提取出来的输入长文本加入到长文本列表:commentList当中
                    }
            );
