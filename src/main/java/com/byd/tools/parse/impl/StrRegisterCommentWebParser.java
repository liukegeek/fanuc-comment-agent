...
/**
 * ClassName: StrRegisterCommentWebParser
 ...
 * Version 1.0
 */
public class StrRegisterCommentWebParser extends BaseWebParser {
    @Override
    public List<Comment> parseDataFromHtml(InputStream sourceInputStream, String charset, String sourceUrl) throws IOException {
        ...
        try {
            ...
            trs.forEach(
                    tr -> {
                        String strRegisterCommentName = tr.select("td").get(1).select("input").first().attr("name");
                        String strRegisterCommentContent = tr.select("td").get(1).select("input").first().attr("value");
                        Comment strRegisterComment = new Comment(Integer.parseInt(strRegisterCommentName.substring(10)), strRegisterCommentContent); //通过name与value信息构造字符串寄存器注释对象。
                        strRegisterComment.setType(CommentType.STRING_REGISTER_COMMENT); //设置注释类型为， 字符串寄存器类型
                        commentList.add(strRegisterComment);//将该行所提取出来的字符串寄存器注释加入到列表:commentList当中
                    }
            );
