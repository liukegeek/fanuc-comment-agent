...
/**
 * ClassName: NumRegisterCommentWebParser
 * Package: com.byd.tools.parse
 * Description:
 * Author: LiuKe
 * Create: 2025/10/28 09:29
 * Version 1.0
 */
public class NumRegisterCommentWebParser extends BaseWebParser {
    @Override
    public List<Comment> parseDataFromHtml(InputStream sourceInputStream, String charset, String sourceUrl) throws IOException {
        ...
        try {
            ...
            trs.forEach(
                    tr -> {
                        String numRegisterCommentName = tr.select("td").get(1).select("input").first().attr("name");
                        String numRegisterCommentContent = tr.select("td").get(1).select("input").first().attr("value");
                        Comment numRegisterComment = new Comment(Integer.parseInt(numRegisterCommentName.substring(10)), numRegisterCommentContent); //通过name与value信息构造长文本对象。
                        numRegisterComment.setType(CommentType.NUM_REGISTER_COMMENT); //设置类型为， 数值寄存器的注释 类型
                        commentList.add(numRegisterComment);//将该行所提取出来的注释加入到注释列表:commentList当中
                    }
            );
