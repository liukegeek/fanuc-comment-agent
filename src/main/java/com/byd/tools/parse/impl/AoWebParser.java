...
/**
 * ClassName: AoWebParser
 * Package: com.byd.tools.parse
 * Description:
 * Author: LiuKe
 * Create: 2025/10/28 09:35
 * Version 1.0
 */
public class AoWebParser extends BaseWebParser {
    @Override
    public List<Comment> parseDataFromHtml(InputStream sourceInputStream, String charset, String sourceUrl) throws IOException {
        ...
        try {
            ...
            trs.forEach(
                    tr -> {
                        String aoCommentName = tr.select("td").get(3).select("input").first().attr("name");
                        String aoCommentValue = tr.select("td").get(3).select("input").first().attr("value");
                        Comment aoComment = new Comment(Integer.parseInt(aoCommentName.substring(10)), aoCommentValue); //通过name与value信息构造长文本对象。
                        aoComment.setType(CommentType.AO); //设置长文本类型为， AO类型
                        commentList.add(aoComment);//将该行所提取出来的输出长文本加入到长文本列表:commentList当中
                    }
            );
