...
/**
 * ClassName: RoWebParser
 * Package: com.byd.tools.parse
 * Description:
 * Author: LiuKe
 * Create: 2025/10/28 09:33
 * Version 1.0
 */
public class RoWebParser extends BaseWebParser {
    @Override
    public List<Comment> parseDataFromHtml(InputStream sourceInputStream, String charset, String sourceUrl) throws IOException {
        ...
        try {
            ...
            trs.forEach(
                    tr -> {
                        String roCommentName = tr.select("td").get(3).select("input").first().attr("name");
                        String roCommentValue = tr.select("td").get(3).select("input").first().attr("value");
                        Comment roComment = new Comment(Integer.parseInt(roCommentName.substring(10)), roCommentValue); //通过name与value信息构造长文本对象。
                        roComment.setType(CommentType.RO); //设置长文本类型为， RO类型
                        commentList.add(roComment);//将该行所提取出来的输出长文本加入到长文本列表:commentList当中
                    }
            );
