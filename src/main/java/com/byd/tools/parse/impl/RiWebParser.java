 ...
 * Version 1.0
 */
public class RiWebParser extends BaseWebParser {
    @Override
    public List<Comment> parseDataFromHtml(InputStream sourceInputStream, String charset, String sourceUrl) throws IOException {
        ...
        try {
            ...
            trs.forEach(
                    tr -> {
                        String riCommentName = tr.select("td").get(1).select("input").first().attr("name");
                        String riCommentValue = tr.select("td").get(1).select("input").first().attr("value");
                        Comment riComment = new Comment(Integer.parseInt(riCommentName.substring(10)), riCommentValue);
                        riComment.setType(CommentType.RI); //设置长文本类型为， RI类型
                        commentList.add(riComment);//将该行所提取出来的输入长文本加入到长文本列表:commentList当中
                    }
            );
