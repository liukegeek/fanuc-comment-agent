...
/**
 * ClassName: NumRegisterValueWebParser
 * Package: com.byd.tools.parse
 * Description:
 * Author: LiuKe
 * Create: 2025/10/28 09:31
 * Version 1.0
 */
public class NumRegisterValueWebParser extends BaseWebParser {

    @Override
    public String generateWritePara(CommentType type, int id, String value, String charset) {
        if (type != CommentType.NUM_REGISTER_VALUE) {
        ...
        try {
            ...
            String sFc = URLEncoder.encode("2", charset);
            /*
             寄存器值修改的参数格式与注释不同，比如 数值寄存器20存储的值更改为999 的url链接为:
                http://192.168.0.1/karel/ComSet?sValue=999&sIndx=20&sRealFlag=-1&sFc=2
             */
            String writePara = "?sValue=" + sValue + "&sIndx=" + sIndx + "&sRealFlag=-1" + "&sFc=" + sFc;
    ...
    public List<Comment> parseDataFromHtml(InputStream sourceInputStream, String charset, String sourceUrl) throws IOException {
        List<Comment> numRegisterValueList = new ArrayList<>();
        ...
        try {
            ...
            trs.forEach(
                    tr -> {
                        ...
                        String numRegisterValueContent = tr.select("td").get(2).select("input").first().attr("value");

                        // 和注释不同，对于寄存器的值，其html文件中的name格式为 name="iVal1"，因此剔除其前4个字符"iVal"才会得到寄存器的编号
                        Register_Value numRegisterValueComment = new Register_Value(Integer.parseInt(numRegisterValueName.substring(4)), numRegisterValueContent); //通过name与value信息构造数值寄存器的值对象。
                        numRegisterValueComment.setType(CommentType.NUM_REGISTER_VALUE); //设置长文本类型为， 数值寄存器值类型
                        numRegisterValueList.add(numRegisterValueComment);//将该行所提取出来的数值寄存器的值加入到长文本列表:commentList当中
                    }
            );
