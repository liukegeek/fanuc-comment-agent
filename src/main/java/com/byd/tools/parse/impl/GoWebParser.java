 ...
 * Version 1.0
 */
public class GoWebParser extends BaseWebParser {
    @Override
    public List<Comment> parseDataFromHtml(InputStream sourceInputStream, String charset, String sourceUrl) throws IOException {
        ...
        try {
            ...
            trs.forEach(
                    tr -> {
                        String goCommentName = tr.select("td").get(3).select("input").first().attr("name");
                        String goCommentValue = tr.select("td").get(3).select("input").first().attr("value");
                        Comment goComment = new Comment(Integer.parseInt(goCommentName.substring(10)), goCommentValue); //通过name与value信息构造长文本对象。
                        goComment.setType(CommentType.GO); //设置长文本类型为， GO类型
                        commentList.add(goComment);//将该行所提取出来的输出长文本加入到长文本列表:commentList当中
                    }
            );
